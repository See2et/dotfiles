...
cd-fzf-find() {
    local DIR=$(find ./ -path '*/\.*' -name .git -prune -o -type d -print 2> /dev/null | fzf-tmux -p 80% +m)
    if [ -n "$DIR" ]; then
        cd $DIR
    fi
}
...
vim-fzf-find() {
    local FILE=$(find ./ -path '*/\.*' -name .git -prune -o -type f -print 2> /dev/null | fzf-tmux -p 80% +m)
    if [ -n "$FILE" ]; then
        ${EDITOR:-nvim} $FILE
    fi
}
...
function cd-fzf-ghqlist() {
    ...
    local REPO=`ghq list -p | sed -e 's;'${GHQ_ROOT}/';;g' |fzf-tmux -p 80% +m`
    if [ -n "${REPO}" ]; then
...
function checkout-fzf-gitbranch() {
    local GIT_BRANCH=$(git branch --all | grep -v HEAD | fzf-tmux -p 80% +m)
...
function buffer-fzf-history() {
    local HISTORY=$(history -n -r 1 | fzf-tmux -p 80% +m)
    BUFFER=$HISTORY
    if [ -n "$HISTORY" ]; then
        CURSOR=$#BUFFER
    else
